...
const Shop = ({ navigation,route }) => {
    ...
    const getShopData = async () => {
        ...
        if (route.params.data == null) {
            var response = await getShop()
        }
        else {
            var response = await get_request("/api/buy-packages-list/" + route.params.data.shop)
        }
        if (response !== "Error") {
    ...
    const renderItem = ({ item, index }) => (
        <View style={{ flex: 1 }}>
            <TouchableOpacity onPress={() => { 
                setfocus(item)
                navigation.navigate("ShopDetail",{data:item,PackageID:item.package_id,Escrow:route.params.data})
