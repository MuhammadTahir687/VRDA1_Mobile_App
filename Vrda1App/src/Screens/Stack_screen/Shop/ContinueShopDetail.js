...
const ContinueShopDetail = ({ navigation,route }) => {
    ...
    const gettingDetails = async ({ text }) => {
        setLoading(true)
        let body = { package_id: packageID, payment_type: text, escrow_link:route.params.Escrow.shop};
        ...
        if (response !== "Error") {
            if (response.data.status === true) {
                ...
                if (text == "bank") {
                    ...
                    console.log("Vreit Payment Type==========",res.vreit.message)
                    setVreitMsg(res.vreit.message);
                    setPurchasingvia(res.vreit.purchasing_via)
                    setEscrow(res.vreit.escrow)
                    setLoading(false);
                } else {
                    setLoading(false);
                }
    ...
    const Submit = async () => {
            ...
            if (response !== "Error") {
                if (response.data.status == true) {
                    ...
                    navigation.navigate("ContinueShop",{data:Escrow})
    ...
    return (
        ...
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <TouchableOpacity onPress={()=>{navigation.navigate("ContinueShop",{data:Escrow})}}>
                                ...
                                <TouchableOpacity activeOpacity={1}>
                                   ...
                                   {selectedValue == "vreit" ?
                                                    <View>
                                                        ...
                                                        {vreitMsg =="You do not have sufficient funds in (Vreit Wallet) to buy a package." ?
                                                        <Text style={{ margin: 10, backgroundColor: "red", borderRadius: 5, padding: 10,color:"white" }}>{vreitMsg}</Text>
                                                        :
...
export default ContinueShopDetail;