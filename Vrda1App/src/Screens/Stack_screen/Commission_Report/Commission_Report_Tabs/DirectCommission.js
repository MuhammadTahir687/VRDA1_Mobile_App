...
const DirectCommission = ({navigation}) => {
    ...
    const renderItem=({item})=>(
        <TouchableOpacity onPress={()=>{setVisible(true),setRes(item)}} style={{height:115,width:"46.9%", backgroundColor: Colors.secondary, borderColor: Colors.white, borderRadius: 10, borderBottomWidth: 2, padding: 8,margin:"1.5%" }}>
            <Text style={{ fontSize: 14, color: Colors.white }}>{item.pin_code}</Text>
            <Text style={{ fontSize: 14, color: Colors.white }}>Total ({item.total?parseFloat(item.total).toFixed(2):"0"})</Text>
            <Text style={{ fontSize: 14, color: Colors.white }}>{item.status ? "Status (" + item.status + ")":"Not Available"}</Text>
            <Text style={{ fontSize: 13, color: Colors.lightgray }}>Activity Date: {item.created_at}</Text>
        </TouchableOpacity>
    )
    return (
        <SafeAreaView style={{flex:1}}>
            ...
            <Dialogs visible={visible} onPress={()=>{setVisible(false)}} title={"Description"}>
                ...
                <Text style={{fontSize:13,paddingHorizontal:10 }}>Pin BV = {res ? res.package_bv:"Not Available"}</Text>
